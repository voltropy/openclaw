...
describe("Invalid engine fallback", () => {
  it("resolveContextEngine() with config pointing to unregistered engine throws with helpful error", async () => {
    await expect(
      resolveContextEngine({
        plugins: { slots: { contextEngine: "nonexistent-engine" } },
      } as any),
  ...
  it("error message includes the requested id and available ids", async () => {
    ...
    try {
      await resolveContextEngine({
        plugins: { slots: { contextEngine: "does-not-exist" } },
      } as any);
      ...
      expect(err.message).toMatch(/Available engines:/);
...
// Mock the LCM register module to avoid pulling in the `postgres` dependency.
// The real registerLcmPlugin imports LcmContextEngine which requires `postgres`.
// We use importOriginal to get the actual registry module (already loaded above)
// so that our stub registers into the same singleton Map.
vi.mock("../plugins/lcm/register.js", async () => {
  const registry = await import("./registry.js");
  return {
    registerLcmPlugin: () => {
      registry.registerContextEngine("lcm", () => ({
        info: { id: "lcm", name: "LCM Stub (test)", version: "0.0.0" },
        async ingest() {
          return { ingested: true };
        },
        ...
        async compact() {
          return { ok: true, compacted: false } as CompactResult;
        },
      }));
    },
  };
...
describe("Initialization guard", () => {
  it("ensureContextEnginesInitialized() is idempotent (calling twice does not throw)", async () => {
    // Use dynamic import to get a fresh module instance (after the mock is applied).
  ...
  it("after init, both 'legacy' and 'lcm' engines are registered", async () => {
    const { ensureContextEnginesInitialized } = await import("./init.js");
